...
```
WolGoWeb_linux_amd64 -port 6699
```

其中参数 `-port` 表示服务端口号，默认是 `6699` 也可以不填。
...
完成部署工作即可开始使用，首先使用浏览器访问 `http://服务器IP或域名:6699`，如果修改了端口号请访问对应的端口。
...
可以直接使用浏览器访问 `http://服务器IP或域名:6699/wol?mac=需要唤醒主机的MAC地址` 当出现以下界面表示唤醒命令发送成功。
...
首先要确保在群晖中已经部署了WolGoWeb，可以访问群晖的IP地址:6699查看是否部署完成。
...
```bash
curl http://127.0.0.1:6699/wol?mac=00-00-00-00-00-00
```

如果你的WolGoWeb不是部署在群晖中，需要将上面命令中的`127.0.0.1:6699`替换成你部署的IP和端口。
...
```
http://192.168.10.10:6699/wol?mac=00-00-00-00-00-00
```

其中`192.168.10.10:6699`是你部署的WolGoWeb，`00-00-00-00-00-00`为你要唤醒的主机MAC地址。
